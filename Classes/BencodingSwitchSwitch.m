...
#import "BencodingSwitchSwitch.h"

@implementation BencodingSwitchSwitch
...
-(void)layoutSubviews
{
    [switchView setFrame:[self bounds]];
...
-(DCRoundSwitch*)switchView
{
	if (switchView==nil)
	{
        ...
        switchView.onText = NSLocalizedString(@"ON", @"");
        switchView.offText = NSLocalizedString(@"OFF", @"");
...
-(void)setOnText_:(id)text
{
    self.switchView.onText=[TiUtils stringValue:text];    
}
-(void)setOffText_:(id)text
{
    self.switchView.offText=[TiUtils stringValue:text];
}
-(void)setOnColor_:(id)color
{
    TiColor *newColor = [TiUtils colorValue:color];
	UIColor *clr = [newColor _color];
    self.switchView.onTintColor = clr;
}
...
-(void)setValue_:(id)value
{
	...
	DCRoundSwitch * ourSwitch = [self switchView];
